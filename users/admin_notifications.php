            ...
            <?php foreach ($adminNotifications as $m) { ?>
              <tr>
                ...
                <td><input type="checkbox" class="maincheck" name="checkbox[<?= $m->id ?>]" value="<?= $m->id ?>" /> <?= echouser($m->user_id) ?>, <?= time2str($m->date_created) ?><br /><?= html_entity_decode($m->message) ?>
                  <br />
                  <?php if ($m->is_read == 1 && $m->is_archived == 0) { ?><i class="fa fa-check"></i> Read<?php }
                                                                                                        if ($m->is_read == 0 && $m->is_archived == 0) { ?> <i class="fa fa-times"></i> Delivered<?php }
                                                                                                                                                                                            if ($m->is_archived == 1) { ?><i class="fa fa-times"></i> Deleted<?php } ?>
                </td>
              </tr>
            <?php  }
        } else { ?>
      ...
      <?php if ($count > 0) { ?>
        <table class="table pull-right" width="20%">
          <tr>
            ...
            <td width="5%">
              <input class='btn btn-primary' type='submit' name="admin_notifications" value='Go!' class='submit' /></td>
          </tr>
        </table>
      <?php } ?>
    </form>
