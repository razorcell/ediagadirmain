...
if (!function_exists('updatePrivate')) {
    function updatePrivate($id, $private)
    {
        $db = DB::getInstance();
        if ($private == 0) {
            $result = $db->query("UPDATE pages SET private = ?,re_auth = ? WHERE id = ?", array($private, 0, $id));
        } else {
...
if (!function_exists('securePage')) {
    function securePage($uri)
    {
            ...
            if (hasPerm([2])) {
                ...
                if (strpos($page, 'usersc/') !== false) {
                    ...
                    if ($q->count() == 1) {
                        ...
                        // $GLOBALS['general-log']->error('Redirect::to' . $us_url_root . $page . '?msg=Page inserted and auto-mapped.');
                        Redirect::to($us_url_root . $page . '?msg=Page inserted and auto-mapped.');
                    }
                }
                // $GLOBALS['general-log']->error('Redirect::to' . $us_url_root . 'users/admin.php?view=page&err=Please+confirm+permission+settings.&new=yes&id=' . $last . '&dest=' . $dest);
                Redirect::to($us_url_root . 'users/admin.php?view=page&err=Please+confirm+permission+settings.&new=yes&id=' . $last . '&dest=' . $dest);
            } else {
                // $GLOBALS['general-log']->error('You must go into the Admin Panel and click the Manage Pages button');
        ...
        if (empty($pageDetails)) {
            ...
            // $GLOBALS['general-log']->error('If user is not logged in, redirect to : ');
            // $GLOBALS['general-log']->error($us_url_root . 'users/login.php?dest=' . $page . '&redirect=' . $dest);
            ...
            if (checkPermission($pagePermissions)) {
                ...
                // $GLOBALS['general-log']->error('homepage = ' . $homepage);//undefined variable $homepage at this step
                // $GLOBALS['general-log']->error('config::get homepage = ' . Config::get('homepage'));
                ...
                // $GLOBALS['general-log']->error('Not permitted. redirect to : ' . $homepage);
