$(function () {
	...
	//----------UPDATE SOURCE INIT-------
	//removing
	//normally it is like this = display: inline-block; from bootstrap-fileinput.css
		...
		if (files.length === 0) {
			...
			$.ajax({
				...
				beforeSend: function () {
					...
					// });
					// App.blockUI({
					// 	target: '#global_dashboard_portlet',
					// 	animate: true
					// });
				},
				success: function (data, textStatus, jqXHR) {
					...
					// // App.unblockUI('#global_dashboard_portlet');
					// App.blockUI({
					// 	target: '#select_file_block',
					// 	animate: true
					// });
	...
	function show_message(msg, timeout = 5000, type = 'danger') {
		$.bootstrapGrowl(msg, {
			ele: '#upper_bar', // which element to append to
	...
	function sendProgressFunction() {
		$.ajax({
			...
			success: function (data, textStatus, jqXHR) {
				...
				$('#progress_bar_updating').attr('style', data.progress_style);
