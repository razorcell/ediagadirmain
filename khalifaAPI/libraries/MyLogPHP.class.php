...
class MyLogPHP
{
	 ...
	 * @param string $tag 
	 */
	function success($value = '', $tag = self::DEFAULT_TAG)
	{

		self::log('WARNING', '<span style="color:blue">' . $value . '</span>', $tag);
	}
	...
	function error($value = '', $tag = self::DEFAULT_TAG)
	{
		...
		self::log('ERROR', '<span style="color:red">' . $value . '</span>', $tag);
	}
	function warning($value = '', $tag = self::DEFAULT_TAG)
	{

		self::log('ERROR', '<span style="color:green">' . $value . '</span>', $tag);
	}
